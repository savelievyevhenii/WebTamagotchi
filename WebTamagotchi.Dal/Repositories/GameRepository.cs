...
public class GameRepository(WebTamagotchiDbContext dbContext) : IGameRepository
{
    ...
    public async Task<Game?> Get(string name, CancellationToken cancellationToken)
    {
        return await dbContext.Games.SingleOrDefaultAsync(x => x.Name == name, cancellationToken);
    }
